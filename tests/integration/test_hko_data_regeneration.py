...
# Paths of the two encoded tables relative to the checkout root (see hko_data/common.py).
JIEQI_BIN: Path = Path('bazi/calendar/hko_data/data/jieqi_encoded.bin')
LUNARDATE_BIN: Path = Path('bazi/calendar/hko_data/data/lunardate_encoded.bin')
...
def test_hko_data_regeneration_via_module_entry(tmp_path: Path) -> None:
  ...
  `python -m bazi.calendar.hko_data.encoder` must rebuild them. That entry imports the
  parent packages first, and only the PEP 562 lazy imports in `bazi/__init__.py` /
  `bazi/calendar/__init__.py` keep it from tripping over the decoder's fail-fast
  ...
  # Sandbox: a full copy of bazi/ (the raw HKO txt travels with it, so encoding stays
  # offline). `__pycache__` is left out — stale bytecode must not shadow copied sources.
  sandbox: Path = tmp_path / 'sandbox'
  shutil.copytree(REPO_ROOT / 'bazi', sandbox / 'bazi', ignore=shutil.ignore_patterns('__pycache__'))
  ...
  # (the level containing bazi/), not the repo root.
  result: subprocess.CompletedProcess[str] = subprocess.run(
    [sys.executable, '-m', 'bazi.calendar.hko_data.encoder'],
    cwd=sandbox,
    capture_output=True,
    text=True,
    timeout=60,
