...
def test_no_bare_dizhi_discovery_calls() -> None:
  ...
  # an executor: every `dizhi_utils.discover` / `discover_mutual` call in that file must sit
  # inside one of the two wrappers -- a call anywhere else silently falls
  # back to the hardcoded defaults (issue #69).
  # 「无裸调」不变量得有执行者：relationship.py 里的两个入口调用必须都在两薄包装体内——
  # 别处的调用会静默回落到硬编码默认（issue #69）。
  wrappers = {'_dz_discover', '_dz_discover_mutual'}
  gated = {'discover', 'discover_mutual'}
