...
# bazi/src/Calendar/CelestialData/Generator.py
#
# Regenerate the committed tables under `CelestialData/data/` from the
...
#   python -m src.Calendar.CelestialData.Generator [--dylib PATH]
...
def _common_header(generated_on: date, dylib_sha256: str, source_api: str) -> list[str]:
  return [
    ...
    '# generated_by: src/Calendar/CelestialData/Generator.py',
...
def main() -> None:
  argparser = argparse.ArgumentParser(description='Regenerate the CelestialData tables from the celestial-calendar library.')
