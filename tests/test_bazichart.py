...
from src.BaziChart import BaziChart, 命盘, DayunDatabase, TransitOptions, TransitDatabase
...
class TestBaziChart(unittest.TestCase):
  ...
  def test_transit_db(self) -> None:
    def __ganzhis_equal(ganzhis1: Sequence[Ganzhi], ganzhis2: Sequence[Ganzhi]) -> bool:
      if len(ganzhis1) != len(ganzhis2):
        return False
      for gz in ganzhis1:
        if gz not in ganzhis2:
          return False
      return True

    chart = BaziChart.random()
    db = chart.transit_db
    birth_gz_year = chart.bazi.ganzhi_date.year
    first_dayun = next(chart.dayun)

    for offset, xiaoyun in enumerate(chart.xiaoyun):
      actual = db.ganzhis(birth_gz_year + offset, TransitOptions.XIAOYUN_LIUNIAN)
      expected = [xiaoyun.ganzhi, BaziUtils.ganzhi_of_year(birth_gz_year + offset)]
      self.assertTrue(__ganzhis_equal(expected, actual))

    self.assertTrue(__ganzhis_equal(db.ganzhis(first_dayun.ganzhi_year, TransitOptions.DAYUN_LIUNIAN), [
      first_dayun.ganzhi,
      BaziUtils.ganzhi_of_year(first_dayun.ganzhi_year)
    ]))
